...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  // Create base query with filter and visibility
  let query: any = {
  ...
  // If user only has ListOwn permission, filter for their notes
  ...
  const can = {
    create: await authService.can(currentUser.id, NoteActions.Create),
...
export const action: ActionFunction = async ({ request }) => {
  await authService.requireUser(request, {
    condition: { any: [NoteActions.Delete, NoteActions.DeleteOwn] },
  });
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold">Notes</h1>
        {can?.create && (
          <Button asChild className="w-full sm:w-auto h-12 text-base">
          <Link prefetch="intent" to="/o/note/new">
      ...
      {isLoading ? (
        ...
        <NoDataMessage type="notes" createLink="/o/note/new" view={view} />
      )}
