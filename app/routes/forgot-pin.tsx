...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!canDoThis) {
    return Response.json(
      {
        error:
          "Vous n'êtes pas autorisé à réinitialiser le mot de passe de cet utilisateur",
      },
      { status: 403 }
    );
  }
  ...
  const token: IToken = await tokenService.generateResetToken(
    user,
    config.auth.tokenTypes.RESET_PIN
  );
  await emailService.sendForgotPINEmail(email, token.token);
  ...
  return [
    {
      name: "description",
      content:
        "Réinitialisez votre PIN pour accéder à la plateforme RH de la Pharmacie Val d'Oise.",
    },
    { title: "PIN oublié - Pharmacie Val d'Oise" },
  ];
}
...
export default function ForgotPIN() {
  ...
  return (
    <div className="flex min-h-screen">
      <div className="flex w-full items-center justify-center p-3 sm:p-4 lg:w-1/2">
        <div className="w-full max-w-[300px] xs:max-w-[320px] sm:max-w-sm">
          ...
          <Card className="w-full">
            <CardHeader className="px-3 py-4 xs:p-4 sm:p-6">
            ...
            <CardContent className="px-3 xs:px-4 sm:px-6">
                ...
                <Form method="post" className="space-y-3 sm:space-y-4">
                  ...
                  >
                    {navigation.state === "submitting"
                      ? "Instructions en cours d'envoie..."
                      : "Envoyer les instructions"}
                  </Button>
                </Form>
              ) : (
                <Alert className="text-xs xs:text-sm sm:text-base">
              ...
              <div className="mt-3 sm:mt-4">
                <Link
                  ...
                  prefetch="intent"
                  disabled={navigation.state === "submitting"}
                  className="inline-flex items-center text-xs xs:text-sm text-muted-foreground hover:text-primary"
                >
                  <ArrowLeft className="mr-2 h-3 w-3 xs:h-4 xs:w-4" />
      ...
      <div className="hidden w-1/2 lg:block bg-[url('https://images.unsplash.com/photo-1562243061-204550d8a2c9?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] bg-cover bg-center">
        <div className="h-full w-full bg-primary/70 backdrop-blur-sm">
          <div className="flex h-full flex-col items-center justify-center px-4 sm:px-6 lg:px-8 text-center text-primary-foreground">
            <PharmacyIcons className="h-12 w-12 mb-6 xs:h-14 xs:w-14 sm:h-16 sm:w-16 sm:mb-8" />
            <h1 className="text-xl xs:text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 sm:mb-4">
              Réinitialisation du PIN
            </h1>
            <p className="text-sm xs:text-base sm:text-lg max-w-lg">
              Nous comprenons que vous puissiez oublier votre PIN. Suivez les
