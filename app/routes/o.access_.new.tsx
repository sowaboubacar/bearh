...
export const loader: LoaderFunction = async ({ request }) => {
  await authService.requireUser(request, {condition: AccessActions.Create});
  return null;
};

export const action: ActionFunction = async ({ request }) => {
  const authenticatedUser = await authService.requireUser(request, {condition: AccessActions.Create});
  ...
  const accessData = {
    ...
    createdBy: authenticatedUser.id as string,
  };

  Object.keys(permissionCategories).forEach((category) => {
    accessData.permissions[category as keyof AccessPermission] = Object.keys(
      permissionCategories[category as keyof AccessPermission].actions
    )
      .filter((action) => formData.get(`${category}-${action}`) === "on")
      .map(
        (action) =>
          action as keyof (typeof permissionCategories)[typeof category]["actions"]
      );
  });

  try {
    const newAccess = await accessService.createOne(accessData);
    return redirect(`/o/access/view/${newAccess.id}`);
  } catch (error) {
    console.error("Error creating access:", error);
    return {
      success: false,
      message: "Une erreur est survenue lors de la création de l'accès.",
    };
  }
};
...
export default function NewAccessForm() {
  ...
  const [permission, setPermission] = useState<AccessPermission>(
    {} as AccessPermission
  );
  ...
  const handlePermissionChange = (
    category: keyof AccessPermission,
    action: string
  ) => {
  ...
  const handleCheckAllCategory = (category: keyof AccessPermission) => {
    setPermission((prevPermissions) => {
      const allActions = Object.keys(permissionCategories[category].actions);
      const updatedCategory = prevPermissions[category]?.length === allActions.length
        ? []
        : allActions;
      return { ...prevPermissions, [category]: updatedCategory };
    });
                ...
                {Object.entries(permissionCategories).map(
                  ([category, { label, actions }]) => (
                    ...
                    >
                      ...
                      >
                        <svg
                          className={`w-4 h-4 text-gray-600 group-hover:text-gray-500 dark:text-gray-400 ${
                        ...
                        <button
                          type="button"
                          className="ml-auto text-xs bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-1 px-2 rounded"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleCheckAllCategory(category as keyof AccessPermission);
                          }}
                        >
                          {permission[category as keyof AccessPermission]?.length ===
                            Object.keys(
                              permissionCategories[category as keyof AccessPermission]
                                .actions
                            ).length
                            ? "Déchocher tous"
                            : "Cocher Tous"}
...
export  function NewAccessForms() {
  const actionData = useActionData<typeof action>();
  const navigation = useNavigation();
  const [activeAccordion, setActiveAccordion] = useState<string | null>(null);
  const [permission, setPermission] = useState<AccessPermission>(
