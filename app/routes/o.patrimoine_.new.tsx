...
export const loader: LoaderFunction = async ({ request }) => {
  const currentLoggedUser = await authService.requireUser(request, {condition: {any: [PatrimoineActions.Create]}})
  ...
  try {
    ...
    return Response.json({ patrimoineTypes, users, positions, teams, departments, hourGroups, access })
  } catch (error) {
...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    const newPatrimoine = await patrimoineService.createOne({
      name,
      type,
    ...
    return redirect(`/o/patrimoine/view/${newPatrimoine.id}`)
  } catch (error) {
    console.error("Error creating patrimoine:", error);
    return Response.json({ success: false, error: 'Échec de la création du patrimoine' }, { status: 400 })
  }
}

export default function NewPatrimoine() {
  const { patrimoineTypes, users, positions, teams, departments, hourGroups, access } = useLoaderData<typeof loader>()
