...
export const loader: LoaderFunction = async ({ request, params }) => {
  const currentLoggedUser = await authService.requireUser(request, {condition: {any: [NewsActions.Edit]}})
  const { id } = params;

  try {
    const news = await newsService.readOne({ id, populate: "attachments" });
...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  try {
    const updatedNews = await newsService.updateOneAfterFindIt(id, {
      title,
      content,
      type,
      isPublished,
    ...
    return redirect(`/o/news/view/${id}`);
  } catch (error) {
    console.error("Error updating news:", error);
    return Response.json(
      { success: false, error: "Échec de la mise à jour de l'actualité" },
      { status: 400 }
    );
  }
};
...
export default function EditNews() {
  const { news, users, teams, departments, positions, hourGroups, accesses, can } =
  ...
  const [isPublic, setIsPublic] = useState(news.isPublic);
        ...
        <Form method="post" onSubmit={handleSubmit}>
          <CardContent className="p-4 sm:p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <Select name="type" defaultValue={news.type} required>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-w-2xl mx-auto w-full p-4">
              ...
              <div className="flex items-center gap-3">
                <Switch
                  id="isEmergency"
                  name="isEmergency"
                  defaultChecked={news.isEmergency}
                  className="h-5 w-10 sm:h-6 sm:w-11"
                />
                <Label
                  htmlFor="isEmergency"
            ...
            <div className="space-y-3">
              ...
              <UploadWidget
                ...
                defaultSelectedDocuments={news.attachments}
                multiple={true}
