...
export const loader = async ({request}: LoaderFunctionArgs) => {
  const currentLoggedUser = await authService.requireUser(request, {condition: {any: [HourGroupActions.Create]}})
...
export const action: ActionFunction = async ({ request }) => {
  const authenticatedUser = await authService.requireUser(request,{
    condition: { any: [HourGroupActions.Create] }});
  ...
  try {
    const newHourGroup = await hourGroupService.createOne({
      note,
     ...
     // @ts-ignore
     await Promise.all(newHourGroup.members?.map((member) => userService.updateCurrentHourGroup(newHourGroup?.id, member)));
   


    return redirect(`/o/hour-group/view/${newHourGroup.id}`)
  } catch (error) {
    console.error("Error creating hour group:", error);
    return Response.json({ success: false, error: 'Échec de la création du groupe d\'heures' }, { status: 400 })
  }
}

export default function NewHourGroup() {
  const { users } = useLoaderData<typeof loader>()
  const actionData = useActionData<typeof action>()
  const navigation = useNavigation()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [workTimes, setWorkTimes] = useState<Array<{date: string, startAt: string, endAt: string, restShouldStartAt?: string, restShouldEndAt?: string}>>([])
  const [selectedDocuments, setSelectedDocuments] = useState<IDocument[]>([]);
  const [uploadWidgetIsBusy, setUploadWidgetIsBusy] = useState(false);


  const addWorkTime = () => {
    setWorkTimes([...workTimes, { date: '', startAt: '', endAt: '', restShouldStartAt: '', restShouldEndAt: '' }])
  }
