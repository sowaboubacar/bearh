...
export const loader: LoaderFunction = async ({ params, request }) => {
  const loggedUser = await authService.requireUser(request, {condition: {any: [DepartmentActions.Edit]}});
  ...
  try {
    ...
    return Response.json({ department, potentialManagers, can });
  } catch (error) {
...
export const action: ActionFunction = async ({ request, params }) => {
  await authService.requireUser(request, {
    condition: { any: [DepartmentActions.Edit]},
  });
  ...
  try {
    const dp = await departmentService.updateOneAfterFindIt(
      params.id as string,
      { name, description, manager, members: memberIds, attachments }
    );
...
export default function EditDepartmentPage() {
  const { department, potentialManagers,can } = useLoaderData<typeof loader>();
  ...
  const [members, setMembers] = useState<string[]>(
    department.members.map((member: any) => member.id)
  );
          ...
          <CardContent className="p-4 sm:p-6 space-y-6">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <UploadWidget 
                ...
                defaultSelectedDocuments={department.attachments}
                multiple={true}
